...
- (instancetype)initForSession:(AIQSession *)session {
    ...
    if (self) {
        ...
        [_pool inTransaction:^(FMDatabase *db, BOOL *rollback) {
            if (! [db executeUpdate:@"UPDATE somessages SET actual = 0"]) {
                AIQLogCError(1, @"Could not deactivate old messages: %@", [db lastError].localizedDescription);
                *rollback = YES;
                return;
            }
            
            FMResultSet *rs = [db executeQuery:@"SELECT solution, identifier, data FROM documents WHERE type = '_backendmessage'"];
            if (! rs) {
                AIQLogCError(1, @"Could not retrieve message documents: %@", [db lastError].localizedDescription);
                *rollback = YES;
                return;
            }
            ...
            while ([rs next]) {
                ...
                NSDictionary *message = [[rs dataForColumnIndex:2] JSONObject];
                ...
                if (activeFrom + timeToLive >= now) {
                    AIQLogCInfo(1, @"Adding message %@ (%@)", identifier, message[kAIQMessageType]);
                    if (! [db executeUpdate:@"INSERT OR REPLACE INTO somessages (solution, identifier, type, revision, created, activeFrom, timeToLive, read)"
                           ...
                           solution,
                           identifier,
                           message[kAIQMessageType],
                           message[@"revision"],
                           message[kAIQMessageCreated],
                           message[kAIQMessageActiveFrom],
                           message[kAIQMessageTimeToLive],
                           @NO]) {
                        AIQLogCError(1, @"Error adding message %@: %@", identifier, [db lastError].localizedDescription);
                        [rs close];
                        *rollback = YES;
                        return;
                    }
                } else if ([message[kAIQMessageType] isEqualToString:@"_comessageresponse"]) {
                    AIQLogCInfo(1, @"Stale message %@ contains a response", identifier);
                    if (! [self expireCOMessageForResponseId:identifier solution:solution inDatabase:db]) {
                        *rollback = YES;
                        [rs close];
                        return;
                    }
            ...
            AIQLogCInfo(1, @"Deleting stale messages");
            if (! [db executeUpdate:@"DELETE FROM somessages WHERE actual = 0"]) {
                AIQLogCError(1, @"Did fail to delete stale messages: %@", [db lastError].localizedDescription);
                *rollback = YES;
                return;
            }
            
            if (count == 0) {
                AIQLogCInfo(1, @"No active messages found");
                _hasMessages = NO;
            } else {
                AIQLogCInfo(1, @"%ld active messages found", (unsigned long)count);
                _hasMessages = YES;
            }
        }];
